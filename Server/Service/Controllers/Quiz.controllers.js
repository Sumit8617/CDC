...
export const createQuiz = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Please provide quiz title and questions" });
    }
    ...
    if (duplicateQuiz) {
      return res
        .status(400)
        .json({ message: "You have already created a quiz with this title." });
    }
...
export const updateQuiz = async (req, res) => {
  try {
    ...
    const isTitleBeingChanged =
      title && title.trim().toLowerCase() !== quiz.title.trim().toLowerCase();
    ...
    if (isTitleBeingChanged) {
      ...
      if (existingQuiz) {
        return res
          .status(400)
          .json({ message: "You already have another quiz with this title" });
      }
    }
