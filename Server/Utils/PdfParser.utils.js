...
const parseQuestionsFromText = async (text, images = []) => {
  ...
  // FIRST: Map images to questions BEFORE processing - based on sequential document order
  // This ensures images stay with the correct questions they belong to
  const totalImages = images.length;
  const totalQuestions = questionBlocks.length;

  console.log(`Total questions: ${totalQuestions}, Total images: ${totalImages}`);

  // Assign images sequentially: image at index 0 to question at index 0, image 1 to question 1, etc.
  ...
  for (let blockIndex = 0; blockIndex < questionBlocks.length; blockIndex++) {
    ...
    // Use pre-calculated image map to assign images to correct question indices
    // This maps images sequentially: image[0] -> question[0], image[1] -> question[1], etc.
    if (questionImage === null && blockIndex < questionImageMap.length) {
